...
fn count_small_ascii(bencher: Bencher) {
    bencher.bench_local(|| black_box(count_reader(black_box(SMALL_ASCII)).unwrap()));
}

#[divan::bench]
fn count_mixed_utf8(bencher: Bencher) {
    bencher.bench_local(|| black_box(count_reader(black_box(MIXED_UTF8.as_bytes())).unwrap()));
}

#[divan::bench]
fn count_large_ascii(bencher: Bencher) {
    bencher.bench_local(|| black_box(count_reader(black_box(LARGE_ASCII.as_slice())).unwrap()));
}
